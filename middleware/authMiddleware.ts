...
const authMiddleware = (
  ...
  console.log(
    "Auth Middleware: Verificando token de autenticação...",
    authHeader
  );
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    ...
    return res
      .status(401)
      .json({
        erro: "Acesso não autorizado. Token não fornecido ou inválido.",
      });
  }
  ...
  if (!secret) {
    ...
    return res
      .status(500)
      .json({ erro: "Erro interno do servidor [Auth Config]." });
  }
