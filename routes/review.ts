...
const reviewCreateSchema = z.object({
  ...
  conteudo: z
    .string()
    .trim()
    .min(1, { message: "O conteúdo do review não pode estar vazio." }),
});
...
const reviewUpdateSchema = z
  ...
  .refine(
    (data) => data.avaliacao !== undefined || data.conteudo !== undefined,
    {
      message:
        "Pelo menos um campo (avaliação ou conteúdo) deve ser fornecido para atualização.",
    }
  );
    ...
    try {
      ...
      if (!result.success) {
        return res.status(400).json({
          erro: "Dados inválidos para o review.",
      ...
      if (existingReview) {
        return res.status(409).json({
          erro: "Você já fez um review para este gibi. Você pode editá-lo.",
        });
      }

      const gibiExists = await prisma.gibi.findUnique({
        where: { id: gibiId },
      });
      if (!gibiExists || gibiExists.excluido) {
        return res
          .status(404)
          .json({ erro: "Gibi não encontrado ou indisponível." });
      }
  ...
  async (req: RequestWithAuth, res: Response) => {
    try {
      ...
      if (!review) {
        return res
          .status(404)
          .json({ message: "Nenhum review seu encontrado para este gibi." });
      }
      ...
      res
        .status(500)
        .json({ erro: "Erro interno ao buscar review do usuário." });
    }
  }
);
  ...
  async (req: RequestWithAuth, res: Response) => {
    try {
      ...
      if (reviewExistente.usuarioId !== usuarioId) {
        return res
          .status(403)
          .json({ erro: "Você não tem permissão para editar este review." });
      }
  ...
  async (req: RequestWithAuth, res: Response) => {
    try {
      ...
      if (isAdminDeletion) {
        ...
        console.log(
          `Usuário ${usuarioId} deletou seu próprio review ${reviewId}`
        );
      }
